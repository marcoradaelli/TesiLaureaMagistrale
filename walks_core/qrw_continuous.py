# Questa classe simula un quantum random walk continuo su un grafo arbitrario.
...
class walker
    ...
    def ottieni_distribuzione_probabilita_a_tempo(self, tempo:float):
        operatore_evoluzione = scipy.linalg.expm(- 1.j * self.grafo_ospite.matrice_adiacenza * tempo)
        stato = operatore_evoluzione.dot(self.stato_iniziale)

        # Calcolo la distribuzione di probabilità.
        return np.abs(np.square(stato))
